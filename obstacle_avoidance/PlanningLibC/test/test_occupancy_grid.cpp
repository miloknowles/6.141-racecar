...
TEST(OccupancyGridTest, testPathOccupiedVectPoint2f) {
	OccupancyGrid ogw = setupOccupancyGrid();
	bool result = ogw.setGridValue(Point2i(1, 1), 127);
	std::vector<Point2f> pts;
	pts.push_back(Point2f(0, 0));
	pts.push_back(Point2f(1, 1));
	pts.push_back(Point2f(1.5, 1.5));
	EXPECT_TRUE(ogw.pathOccupiedVectPoint2f(pts));
}
...
TEST(OccupancyGridTest, testDynamicUpdate) {
	OccupancyGrid ogw = setupOccupancyGrid();

	nav_msgs::OccupancyGrid dynGrid;
	dynGrid.info.width = 10;
	dynGrid.info.height = 10;
	dynGrid.info.resolution = 0.1;
	dynGrid.info.origin.position.x = 0.0;
	dynGrid.info.origin.position.y = 0.0;

	for (int i = 0; i < 100; i++) {
		dynGrid.data.push_back(100);
	}
	Pose3 tf(Point2f(1, 2), 0);

	ogw.dynamicUpdate(dynGrid, tf);

	// Check corners and one point in middle.
	EXPECT_EQ(100, ogw.getGridValue(Point2i(10, 20)));
	EXPECT_EQ(100, ogw.getGridValue(Point2i(19, 29)));
	EXPECT_EQ(100, ogw.getGridValue(Point2i(19, 20)));
	EXPECT_EQ(100, ogw.getGridValue(Point2i(10, 29)));
	EXPECT_EQ(100, ogw.getGridValue(Point2i(15, 25)));
}
