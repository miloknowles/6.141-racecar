...
class ParticleFiler():
    ...
    def __init__(self):
        ...
        self.OCCUPANCY_INC_PER_SCAN = Utils.getParamOrFail("/mapping/occupancy_inc_per_scan")
        self.OCCUPANCY_DECAY_PER_SCAN = Utils.getParamOrFail("/mapping/occupancy_decay_per_scan")
        self.DILATION_RADIUS_METERS = Utils.getParamOrFail("/mapping/dilation_radius_meters")
        self.MAX_RANGE_METERS = Utils.getParamOrFail("/mapping/max_range_meters")
        self.MIN_RANGE_METERS = Utils.getParamOrFail("/mapping/min_range_meters")
        self.LOCAL_MAP_RESOLUTION   = Utils.getParamOrFail("/mapping/local_map_resolution")
        ...
        localMapWidth = int((self.LOCAL_MAP_XLIM[1] - self.LOCAL_MAP_XLIM[0]) / self.LOCAL_MAP_RESOLUTION)
        localMapHeight = int((self.LOCAL_MAP_YLIM[1] - self.LOCAL_MAP_YLIM[0]) / self.LOCAL_MAP_RESOLUTION)

        self.local_map_origin_pixel = np.array([
            (0-self.LOCAL_MAP_XLIM[0])/self.LOCAL_MAP_RESOLUTION,
            (0-self.LOCAL_MAP_YLIM[0])/self.LOCAL_MAP_RESOLUTION]).astype(int)
        ...
        self.map_debug_pub = rospy.Publisher('/map_debug/', OccupancyGrid, queue_size=1)
        self.dilated_map_pub = rospy.Publisher('/dilated_map', OccupancyGrid, queue_size=1)
        self.local_map_pub = rospy.Publisher('/local_map', OccupancyGrid, queue_size=1)
        ...
        self.cartesian_debug = rospy.Publisher("/cartesian_debug", Marker, queue_size=1)
    ...
    def get_omap(self):
        ...
        self.dilated_map_msg = rospy.ServiceProxy(map_service_name, GetMap)().map
    ...
    def init_dynamic_map(self, path_to_map="/home/racecar/racecar_ws/src/obstacle/maps/basement_fixed.png"):
        ...
        self.dynamic_map = np.flip(self.dynamic_map, 0).astype(np.uint8)
        self.base_map_mask = self.dynamic_map < 50
    ...
    def update(self):
        ...
        if self.lidar_initialized and self.odom_initialized and self.map_initialized:
            if self.state_lock.locked():
                ...
                # Call the appropriate mapping update function!
                if self.USE_LOCAL_MAP:
                    self.update_local_map(self.laser_angles, self.laser)
                else:
                    self.update_dynamic_map(self.inferred_pose, self.laser_angles, self.laser)

                if self.iters % ITERS_PER_UPDATE == 0 and UPDATE_OMAP:
                    self.update_range_method_map()
                self.state_lock.release()
                t2 = time.time()
    ...
    def update_local_map(self, angles, ranges):
        """
        Updates a small map in the local car frame.
        Timing this showed about 0.004-0.1 sec for this function to finish.
        """
        # Local map pixels.
        self.coords_disc = self.pol2cart(0, 0, 0, ranges, angles, self.LOCAL_MAP_RESOLUTION) + self.local_map_origin_pixel

        # Decrement all cells by a fixed amount to model time-uncertainty.
        self.local_map[self.local_map >= self.OCCUPANCY_DECAY_PER_SCAN] -= self.OCCUPANCY_DECAY_PER_SCAN
        self.local_map[self.local_map < self.OCCUPANCY_DECAY_PER_SCAN] = 0

        thickness = 1 # TODO

        # Clear out free space.
        for coord in self.coords_disc:
            cv.line(self.local_map, (0,0), (coord[0],coord[1]), 0, thickness)
        
        # Mark endpoints.
        for coord in self.coords_disc:
            self.local_map[coord[1]][coord[0]] = min(127, self.local_map[coord[1]][coord[0]] + self.OCCUPANCY_INC_PER_SCAN)

        # Process image into an OccupancyGrid msg.
        self.local_map_dilated = cv.dilate(self.local_map, self.dilate_kernel)

        # Flatten into row-major order.
        self.local_map_msg.data = self.local_map_dilated.flatten()
        self.local_map_pub.publish(self.local_map_msg)

    def update_dynamic_map(self, pose, angles, ranges):
        # Get coords in world frame.
        coords = self.pol2cart(pose[0], pose[1], pose[2], ranges, angles, self.map_info.resolution)

        car_pixel = np.array([pose[0] / self.map_info.resolution, pose[1] / self.map_info.resolution]).astype(int)

        # Decrement all points not in the base map.
        if self.base_map_mask is not None:
            self.dynamic_map[np.logical_and(self.base_map_mask, self.dynamic_map > self.OCCUPANCY_DECAY_PER_SCAN)] -= self.OCCUPANCY_DECAY_PER_SCAN
            self.dynamic_map[np.logical_and(self.base_map_mask, self.dynamic_map <= self.OCCUPANCY_DECAY_PER_SCAN)] = 0.0

        # Clear out free space.
        thickness = 1
        for coord in coords:
            cv.line(self.dynamic_map, (car_pixel[0], car_pixel[1]), (coord[0], coord[1]), 0, thickness)

    	# Make sure that original map stays.
    	self.dynamic_map[self.base_map_mask == 0] = 127

        # Mark endpoints.
        for coord in coords:
            if coord[0] < self.map_info.width and coord[1] < self.map_info.height and \
                (self.dynamic_map[coord[1]][coord[0]] <= 127-self.OCCUPANCY_INC_PER_SCAN):
                self.dynamic_map[coord[1]][coord[0]] += self.OCCUPANCY_INC_PER_SCAN

        # Publish.
        self.dilated_map_msg.data = cv.dilate(self.dynamic_map, self.dilate_kernel).flatten().tolist()
        self.dilated_map_pub.publish(self.dilated_map_msg)
    ...
    def pol2cart(self, x0, y0, phi0, rho, phi, resolution):
        """
